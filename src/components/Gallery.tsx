...
import { GALLERY, randomParams } from "@/lib/patterns";
  ...
  return (
    <div>
      <div className="mb-2 flex items-center justify-between">
        <span className="text-sm font-medium text-neutral-200">Empezá con algo</span>
        <button
          ...
          title="Genera una forma al azar (siempre imprimible)"
        >
          ✦ Sorprendeme
        </button>
      </div>
      <div className="grid grid-cols-3 gap-2">
        {GALLERY.map((g) => (
          ...
          >
            ...
            <div className="truncate px-2 py-1 text-[11px] text-neutral-300">{g.name}</div>
          </button>
        ))}
      </div>
    </div>
